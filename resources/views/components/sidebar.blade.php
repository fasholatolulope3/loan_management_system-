...
<aside x-data="{ mobileMenuOpen: false, collapsed: false }" class="relative">
    ...
    <div :class="{ 'translate-x-0': mobileMenuOpen, '-translate-x-full': !mobileMenuOpen, 'w-64': !collapsed, 'w-20': collapsed }"
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar overflow-x-hidden">
            ...
            <!-- Global Section -->
            <x-sidebar-item icon="home" route="dashboard" :active="request()->routeIs('dashboard')">General
                Overview</x-sidebar-item>
            ...
            @if (auth()->user()->role === 'admin')
                ...
                <x-sidebar-item icon="building-office-2" route="admin.centers.index"
                    :active="request()->routeIs('admin.centers.*')">Collation
                    Centers</x-sidebar-item>
                <x-sidebar-item icon="users" route="users.index" :active="request()->routeIs('users.*')">Staff
                    Management</x-sidebar-item>
                <x-sidebar-item icon="squares-2x2" route="loan-products.index"
                    :active="request()->routeIs('loan-products.*')">Credit
                ...
                <x-sidebar-dropdown icon="shield-check" label="Approval Face" :active="request()->routeIs('loans.*')">
                    <x-sidebar-item route="loans.index" :active="request()->routeIs('loans.index')" sub-item>Master
                        Portfolio</x-sidebar-item>
                    <x-sidebar-item route="loans.index" :active="request()->is('loans?status=pending')" sub-item>Pending
                ...
                <x-sidebar-item icon="finger-print" route="audit-logs.index"
                    :active="request()->routeIs('audit-logs.*')">Trace Audit
                    Logs</x-sidebar-item>
                <x-sidebar-item icon="adjustments-horizontal" route="settings"
                    :active="request()->routeIs('settings')">Global
            ...
            @if (auth()->user()->role === 'officer')
                ...
                <x-sidebar-item icon="identification" route="guarantors.index"
                    :active="request()->routeIs('guarantors.*')">Guarantor
                ...
                <x-sidebar-item icon="document-text" route="loans.create" :active="request()->routeIs('loans.create')">New
                    Credit
                    Proposal</x-sidebar-item>
                <x-sidebar-item icon="clock" route="loans.index" :active="request()->is('loans?status=pending')">Active
                    Proposals</x-sidebar-item>
